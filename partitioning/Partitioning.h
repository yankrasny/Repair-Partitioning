...
public:
	 	...
	 	method(method)
	{
		...
		fragments = std::vector<std::vector<FragInfo > >();
	 	...
	 	unsigned maxArraySize = numVersions * MAX_NUM_FRAGMENTS_PER_VERSION;

		this->offsets = new unsigned[maxArraySize];

		this->versionSizes = new unsigned[numVersions];
	...
	// The offsets for all versions as one long array
	unsigned* getOffsets()
	{
		return offsets;
	}

	// The sizes of each version, which helps us parse the array of offsets
